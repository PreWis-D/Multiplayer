...
public class PlayerController : MonoBehaviourPunCallbacks
{
    ...
    public event UnityAction Crouched;
    ...
    public Transform rigAim;
    public Transform aim;
    private Vector3 startAimPosition;
    private Vector3 detectorPosition = new Vector3(0,0,0.5f);
    ...
    {
        base.OnEnable();
        playerInput.SprintKeyClicked += OnSprintKeyClicked;
        playerInput.SprintKeyUp += OnSprintKeyUp;
        playerInput.JumpKeyClicked += OnJumpKeyClicked;
        playerInput.CrouchKeyClicked += OnCrouchKeyClicked;
    }
    ...
    private void Update()
    {
        ...
        if (cameraCanMove)
        {
            yaw = transform.localEulerAngles.y + /*Input.GetAxis("Mouse X")*/playerInput.CameraX * mouseSensitivity;

            if (!invertCamera)
            {
                pitch -= mouseSensitivity * /*Input.GetAxis("Mouse Y")*/playerInput.CameraY;
            }
            else
            {
                // Inverted Y
            ...
            if (isCrouched)
                pitch = Mathf.Clamp(pitch, -maxLookAngle, maxLookAngle / 2);
            else
                pitch = Mathf.Clamp(pitch, -maxLookAngle, maxLookAngle);

            transform.localEulerAngles = new Vector3(0, yaw, 0);
            playerCamera.transform.localEulerAngles = new Vector3(pitch, 0, 0);
            rigAim.position = aim.position;
        }
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            if (enableSprint && isSprinting && sprintRemaining > 0f && !isSprintCooldown && isCrouched == false)
            {
    ...
    public void ChangeMoveCamera(bool value)
    {
        cameraCanMove = value;
    }
    ...
    private void OnSprintKeyClicked()
    {
        isSprinting = true;
    }
    ...
    {
        jointOriginalPos = new Vector3(jointOriginalPos.x, head.position.y + offset, jointOriginalPos.z);
        if (isWalking)
        {
...
public class PlayerContollerEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        fpc.playerInput = (PlayerInput)EditorGUILayout.ObjectField(new GUIContent("PlayerInput", "Camera attached to the controller."), fpc.playerInput, typeof(PlayerInput), true);
        fpc.playerCamera = (Camera)EditorGUILayout.ObjectField(new GUIContent("Camera", "Camera attached to the controller."), fpc.playerCamera, typeof(Camera), true);
        fpc.rigAim = (Transform)EditorGUILayout.ObjectField(new GUIContent("Rig", "RigAim"), fpc.rigAim, typeof(Transform), true);
        fpc.aim = (Transform)EditorGUILayout.ObjectField(new GUIContent("Aim", "Aim"), fpc.aim, typeof(Transform), true);
        ...
        fpc.head = (Transform)EditorGUILayout.ObjectField(new GUIContent("Head", "Joint object position is moved while head bob is active."), fpc.head, typeof(Transform), true);
        fpc.joint = (Transform)EditorGUILayout.ObjectField(new GUIContent("Camera Joint", "Joint object position is moved while head bob is active."), fpc.joint, typeof(Transform), true);
        fpc.bobSpeed = EditorGUILayout.Slider(new GUIContent("Speed", "Determines how often a bob rotation is completed."), fpc.bobSpeed, 1, 20);
        fpc.offset = EditorGUILayout.FloatField(new GUIContent("offset", "Determines how often a bob rotation is completed."), fpc.offset);
